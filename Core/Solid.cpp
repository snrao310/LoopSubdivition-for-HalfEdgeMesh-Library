...
void Solid::edgeSwap(Edge *e, Vertex *v1, Vertex *v2)
{
	...
	int vid[3],k;
	tVertex w[3];
	w[0] = v2; w[1] = v[0][0]; w[2] = v[0][1];
	for( k = 0; k < 3; k ++ )
	{
		vid[k] = w[k]->id(); 
	}		
	//printf("\n%f %f %f\n",w[0]->point()(0),w[0]->point()(1),w[0]->point()(2));
	//printf("%f %f %f\n",w[1]->point()(0),w[1]->point()(1),w[1]->point()(2));
	//printf("%f %f %f\n",w[2]->point()(0),w[2]->point()(1),w[2]->point()(2));
	//printf("%f %f %f\n",v[2]->point()(0),v[2]->point()(1),v[2]->point()(2));
	Face * nf = createFace( vid, ++max_fid );
	new_faces.Append( nf );
	//printf("im here\n");
	
	w[0] = v2; w[1] = v[0][1]; w[2] = v[0][2];
	for( k = 0; k < 3; k ++ )
	{
		vid[k] = w[k]->id(); 
	}	
...
Vertex * Solid::edgeSplit( Edge * e ,Edge *splitSwap[], int maxfcid)
{
	//find the max_id for vertices;
...
int flag[2]={0};
	...
	for( int j = 0; j < 2; j ++ )
	{
		...
		{
			flag[j]=1;
			//printf("this happened here maxfcid=%d f->id=%d\n",maxfcid,f->id());
		}
		m_faces.remove( f );
		delete f;
